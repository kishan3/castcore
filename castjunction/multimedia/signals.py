...
def update_multimedia(sender, instance, *args, **kwargs):
    ...
    if instance.content_type.name == "person":
        ...
        if (
            user.images.filter(image_type=choices.PRIMARY).count() == 0
            and instance.image_type == choices.PRIMARY
        ):
            increase_percentage(user, "images")

        if (
            user.images.filter(image_type=choices.COVER).count() == 0
            and instance.image_type == choices.COVER
        ):
            increase_percentage(user, "images")
