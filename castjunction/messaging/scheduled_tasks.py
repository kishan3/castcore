...
def broadcast_approved_jobs():
    ...
    today = today.replace(hour=broadcast_hour,
                          minute=broadcast_minute,
                          second=broadcast_second,
                          microsecond=broadcast_microsecond)
    yesterday = today - timedelta(days=1)

    all_jobs = Job.objects.filter(status='A',
                                  updated_at__range=(yesterday, today))
    all_users = Person.objects.filter(preference__email_notification=True)

    if all_jobs:
        for gender, _ in choices.GENDER_CHOICES:
            filtered_jobs = all_jobs.filter(required_gender__in=[gender, choices.NOT_SPECIFIED])
            if filtered_jobs:
                filtered_users = list(all_users.filter(gender=gender
                                                       ).exclude(email=None
                                                                 ).values_list('email', flat=True)
                                      )
                context = {"job_count": filtered_jobs.count(),
                           "jobs": filtered_jobs,
                           "domain": django_settings.DOMAIN,
                           "url": django_settings.JOB_OPPORTUNITIES_URL}
                JobApprovedEmailNotification(receiver=filtered_users, context=context).send()
