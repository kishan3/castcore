...
class SearchableField(models.Model):
    ...
    model_name = models.CharField(
        max_length=255, help_text="model name on which search is performed."
    )
    field_name = models.CharField(
        max_length=255, help_text="field of the model on which search is performed."
    )
    value = models.CharField(max_length=255, help_text="Value for this field.")
...
class UserIncentives(mixins.CommonFieldsMixin):
    ...
    job_post = models.CharField(
        max_length=255, default="500", help_text="Incentives for job post."
    )
    user_paid = models.CharField(
        max_length=255, default="500", help_text="Incentives for user payment."
    )
...
class PersonType(models.Model):
    ...
    person_type = models.CharField(
        max_length=30, choices=PERSON_TYPE_CHOICES, blank=False, default=TALENT
    )
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    nationality = models.ForeignKey(
        ...
        blank=True,
        help_text=_(
            "This will be location of head office in case of production house or company."
        ),
    )
    ...
    email_daily_updates = models.BooleanField(
        default=False, help_text="Whether to mail updates daily?"
    )

    sms_passcode = models.PositiveIntegerField(
        blank=True, null=True, verbose_name="Passcode for user's phone verification"
    )

    is_email_verified = models.BooleanField(
        default=False, verbose_name="Email of user is verified?"
    )
    is_phone_verified = models.BooleanField(
        default=False, help_text="Phone number of user is verified?"
    )
    is_profile_approved = models.BooleanField(
        default=False, help_text="Profile of user is approved by StageRoute?"
    )
    ...
    interested_professions = SelectMultipleField(
        max_length=50, default=choices.OTHER, choices=choices.JOB_TYPE_CHOICES
    )
...
class Person(User):
    ...
    associated_with_agent = models.BooleanField(
        default=False, help_text="Whether user has agents working for him/her?"
    )
    incentive_plan = models.ForeignKey(
        UserIncentives, null=True, blank=True, help_text="Incentive plan details."
    )
...
class Bio(models.Model):
    ...
    person = models.OneToOneField(
        Person, related_name="bio", related_query_name="bio", on_delete=models.CASCADE
    )
...
class Language(models.Model):
    ...
    person = models.ManyToManyField(
        Person, related_name="known_languages", related_query_name="known_language"
    )
...
class Institute(models.Model):
    ...
    established_year = models.DateField(
        null=True, blank=True, help_text=_("Date when institute was established.")
    )
...
class Education(models.Model):
    ...
    institute = models.ForeignKey(
        Institute, related_name=related_name, related_query_name=related_query_name
    )

    start_date = models.DateField(
        null=True, blank=True, help_text=_("Date when person started the course.")
    )
    end_date = models.DateField(
        null=True, blank=True, help_text=_("Date when person ended the course.")
    )
    ...
    social_activites = models.TextField(
        ...
        blank=True,
        help_text=_(
            "Description about user's extracurricular and other social activites."
        ),
    )
...
class Experience(models.Model):
    ...
    person = models.ForeignKey(
        Person, related_name="experiences", related_query_name="experience"
    )
    ...
    character_name = models.CharField(
        max_length=255, null=True, blank=True, help_text="Name of character."
    )
    experience_type = models.PositiveSmallIntegerField(
        choices=EXPERIENCE_TYPE, null=True, blank=True
    )
    ...
    description = models.TextField(
        null=True, blank=True, help_text=_("Desctiption about user's experience.")
    )


class CompanyType(models.Model):
    PRODUCTION_HOUSE = "production_house"
    COMPANY_TYPE_CHOICES = ((PRODUCTION_HOUSE, "production house"),)
...
class Bank(models.Model):
    name = models.CharField(
        max_length=255, null=True, blank=True, help_text="Name of bank."
    )
    ...
    pan_number = models.CharField(
        max_length=10, null=True, blank=True, help_text="Pan card number of company."
    )
...
class Company(User):
    ...
    typ = models.ManyToManyField(CompanyType, related_name=related_name)
    members = models.ManyToManyField(
        Person, related_name=related_name, related_query_name=related_query_name
    )
    company_email = models.EmailField(_("email address"), null=True, blank=True)
    company_phone = models.CharField(
        unique=True,
        max_length=10,
        null=True,
        blank=True,
        help_text=_("Company's phone number object."),
    )
    company_website = models.URLField(null=True, blank=True)
    bank = models.ForeignKey(
        Bank, null=True, blank=True, help_text="Bank details for the company."
    )
