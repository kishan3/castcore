...
def send_verification_reminder_sms(user):
    ...
    if not user.is_phone_verified:
        send_sms(user.phone, REMINDER_VERIFY_PHONE_SMS.format(**{'otp_url':
...
def user_signed_up_(request, user, **kwargs):
    ...
    if user.email:
        ...
        context['url'] = django_settings.ACTIVATION_URL.format(**context)
        if user._password:
            context['password_line'] = EMAIL_PASSWORD_LINE.format(**{'password': user._password})
    ...
    if sender.__name__ in list_of_models:
        if created:
            ...
            create_limited_credit_account(
                user=instance,
                account_type=django_settings.TOKEN_ACCOUNT)
            create_limited_credit_account(
                user=instance,
                account_type=django_settings.REIMBURSEMENT_ACCOUNT)
            action.send(instance, verb=u"joined stageroute.")
            UserPreference.objects.create(
                user=instance)
        else:
            if not Action.objects.filter(target_object_id=instance.id,
                                         description='Updated Profile',
                                         timestamp__gte=datetime.now(tz=timezone.utc) - timedelta(minutes=2)).exists():
            ...
            if instance.is_profile_approved and instance.preferences.email_notification:
                ...
                ProfileApprovedEmailNotification(instance.email,
