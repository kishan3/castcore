...
from .models import (User, Person, Experience, Education, Institute, Bio,
                     Language, Skill, SearchableField, UserPreference)
from .utils import (is_user_casting_director, get_email_context,
                    PERCENTAGE_BASIC_DETAILS_FIELDS,
                    update_users_profile_percentage,
                    increase_percentage)
from .permissions import (IsOwnerOrReadOnly, IsCastingDirector,
                          IsSupportGroupMember)

from .adapters import (FacebookOAuth2AdapterFixed, TwitterOAuth2AdapterFixed,
                       GoogleOAuth2AdapterFixed)
from .serializers import (UserSerializer, CustomSocialLoginSerializer,
                          TwitterLoginSerializer, LikeSerializer,
                          PersonSerializer, CompanySerializer,
                          ExperienceSerializer, EducationSerializer,
                          BioSerializer, LanguageSerializer,
                          SkillSerializer, InstituteSerializer,
                          CitySerializer, CountrySerializer,
                          EducationSearchSerializer,
                          ExperienceSearchSerializer,
                          SearchableFieldSerializer,
                          PasswordResetConfirmSerializer,
                          ReferralResponseSerializer, TokenSerializer,
                          UidSerializer, UserPreferenceSerializer,
...
class LoginView(BaseLoginView):
    ...
    def get_response(self):
        return Response(
            self.response_serializer(self.token,
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        for key in request.data.keys():
            if key in PERCENTAGE_BASIC_DETAILS_FIELDS.keys() and getattr(instance, key) in ['NS', None]:
...
class EducationViewSet(mixins.CreateModelMixin,
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(education_instance, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
...
class EducationSearchView(HaystackViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if 'field_of_study' in request.GET:
            sqs = SearchQuerySet().autocomplete(field_of_study_auto=request.GET.get('field_of_study', ''))[:5]
        elif 'degree' in request.GET:
            sqs = SearchQuerySet().autocomplete(degree_auto=request.GET.get('degree', ''))[:5]
        else:
...
class ExperienceSearchView(HaystackViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if 'production_house' in request.GET:
            sqs = SearchQuerySet().autocomplete(production_house_auto=request.GET.get('production_house', ''))[:5]
        elif 'role' in request.GET:
            sqs = SearchQuerySet().autocomplete(role_auto=request.GET.get('role', ''))[:5]
        else:
            return super().list(request, *args, **kwargs)
...
class SearchFieldView(HaystackViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            allowed_keys = SearchableField.objects.values_list('field_name', flat=True).distinct()
            ...
            if search_field not in allowed_keys:
                raise ValidationError("allowed keys to search are: {}".format(allowed_keys))
            search_value = request.GET.get(search_field, '')

            sqs = SearchQuerySet().autocomplete(
                value_auto=search_value,
...
class BioViewSet(mixins.UpdateModelMixin, viewsets.GenericViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user.user_type == User.PERSON:
            ...
            raise ValidationError("User object other then person type does not have bio.")
...
class LikeViewSet(NestedViewSetMixin, mixins.CreateModelMixin,
                  viewsets.GenericViewSet):
    """Likes view set."""
...
class UserLikeViewSet(LikeViewSet):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(users, many=True, context={'request': self.request})
    ...
    def update(self, request, *args, **kwargs):
        ...
        if languages:
            for language in languages:
                language, created = Language.objects.get_or_create(language_name=language)
...
class InstituteViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = (filters.SearchFilter,)
    search_fields = ('institute_name', 'id',)
...
class PasswordResetView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not email and not phone_number:
            raise ValidationError("Please provide an email address or phone number to proceed to reset password.")
        if email:
            ...
            context['url'] = django_settings.PASSWORD_RESET_CONFIRM_URL.format(**context)
        ...
        if phone_number:
            ...
            try:
                ...
                send_sms(phone_number, RESET_PASSWORD_OTP_SMS.format(**{'otp': sms_passcode}))
...
class PasswordResetConfirmView(ActionViewMixin, generics.GenericAPIView):
    ...
    def action(self, serializer):
        ...
        return Response({'results': 'Your password has been changed successfully.'}, status=status.HTTP_200_OK)
...
class VerifyEmailView(ActionViewMixin, generics.GenericAPIView):
    ...
    def action(self, serializer):
        ...
        return Response({'results': 'Your email id has been verified successfully.'},
                        status=status.HTTP_200_OK)


class ContactUsView(mixins.CreateModelMixin,
                    viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            send_mail.delay(subject, body, sender, receivers,
                            content_type='html')
        except Exception as e:
            raise e
...
class PersonFilter(django_filters.FilterSet):
    ...
    def filter_max_age(self, queryset, value):
        if value:
            birth_date_lower_limit = datetime.today() - relativedelta(years=int(value)+1)
...
class PersonViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    ordering_fields = ('date_joined', 'stageroute_score',
...
class UserEarningsView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if is_casting_director:
            ...
            paid_users_transactions = self._filter_transactions(transactions, "user_paid")
            paid_users_earnings = self._total_amount(paid_users_transactions)

            application_transactions = self._filter_transactions(transactions, "application")
            ...
            jobs['pendin_approval'] = posted_jobs.filter(
                status=choices.PENDING_APPROVAL).count()
            jobs['closed'] = posted_jobs.filter(status=choices.CLOSED).count()
            referred_users = ReferralResponse.objects.filter(
                referral__in=request.user.referral_codes.all())
            # application count
            applications = Application.objects.filter(
                job__in=posted_jobs).exclude(
                    state__in=[State.PIPELINED, State.IGNORED]).count()
            paid_users_count = referred_users.filter(action="CASTING_DIRECTOR_PAID").count()
...
class ApplicationsOnJobsViewSet(
        mixins.ListModelMixin,
        viewsets.GenericViewSet):
    permission_classes = (IsCastingDirector,)
    serializer_class = ApplicationDetailSerializer
    filter_backends = (filters.DjangoFilterBackend, filters.OrderingFilter,)
    filter_fields = ("state", "job_id",)
    ordering_fields = ('created_at',)
    ordering = ('-created_at',)

    def get_queryset(self):
        applications = Application.objects.filter(
            job__created_by=self.request.user).exclude(
            state__in=[State.IGNORED, State.PIPELINED])
        return applications
    ...
    def verify_phone(self, request, pk=None):
        ...
        if user.is_phone_verified:
            ...
            try:
                ...
                return Response({'response': 'Please enter passcode with numbers only.'}, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
                raise e
            ...
            if passcode_entered == user.sms_passcode:
                ...
                return Response({'response': 'Thank you for verifying your phone number.',
                                 }, status=status.HTTP_200_OK)
            else:
                return Response({'response': 'passcode entered is invalid.'}, status=status.HTTP_400_BAD_REQUEST)


class CheckUserView(viewsets.GenericViewSet):
    permission_classes = (IsAuthenticated, IsCastingDirector,)
    serializer_class = UserSerializer
...
class NotifyUserView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not any([email, phone, uid]):
            raise ValidationError(detail="At least one of email, phone or uid is required.")
        if uid:
        ...
        if email:
            ...
            if not all([subject, email_message]):
                raise ValidationError("Subject and email_message both required to send an email.")
            context = {
                "subject": subject,
