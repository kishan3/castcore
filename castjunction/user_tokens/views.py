...
class PaymentDetailsView(mixins.OrderPlacementMixin, generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.get("ResponseCode") == "0":
            source_type, __ = models.SourceType.objects.get_or_create(name="EBSGateway")
            ...
            for line in lines:
                token = line.product.attribute_values.filter(
                    attribute__name="allocate_tokens"
                )[0].value_integer
            ...
            if referralresponse:
                ...
                if casting_director:
                    if (
                        datetime.today().date() - user.date_joined.date()
                    ).days <= django_settings.CA_INCENTIVE_THRESHOLD:
                        ...
                        try:
                            credit_to_reimbursement_account(
                                referrer_user, amount, merchant_reference="user_paid"
                            )
...
class InviteView(generics.ListCreateAPIView):
    ...
    def _generate_referral_link(self, user):
        ...
        return "{0}&{1}".format(
            ...
            django_settings.INVITE_URL.format(**{"referral_code": referral_code}),
        )
...
class PaymentInitialDataView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            raise NotFound(
                "Order with id {} does not exists.".format(kwargs.get("order_id"))
            )
...
class TokenView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            token_account = Account.objects.get(
        ...
        return Response(
            {
                "results": [
                    self.get_serializer(token_account).data,
