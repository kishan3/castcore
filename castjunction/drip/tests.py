...
class DripsTestCase(TestCase):
    ...
    def test_custom_drip_static_datetime(self):
        ...
        QuerySetRule.objects.create(
            ...
            field_name="date_joined",
            lookup_type="lte",
            field_value=(timezone.now() - timedelta(days=8)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        )
        drip = model_drip.drip
    ...
    def test_custom_drip_static_now_datetime(self):
        ...
        QuerySetRule.objects.create(
            ...
            field_name="date_joined",
            lookup_type="gte",
            field_value=(timezone.now() - timedelta(days=1)).strftime(
                "%Y-%m-%d 00:00:00"
            ),
        )
        drip = model_drip.drip
    ...
    def test_apply_annotations_with_count(self):
        ...
        self.assertEqual(
            list(qs.query.aggregate_select.keys()), ["num_profile_user_groups"]
        )
