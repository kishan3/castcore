...
class BaseTest(TestCase):
    ...
    def setUp(self):
        ...
        settings.LANGUAGE_CODE = (
            "en"  # do not bother about translation ; needed for the server side
        )
...
class ViewTest(BaseTest):
    ...
    def test_home(self):
        ...
        self.assertRedirects(
            response, reverse("postman:inbox"), status_code=301, target_status_code=302
        )
    ...
    def check_message(
        self, m, is_anonymous=False, subject="s", body="b", recipient_username="bar"
    ):
    ...
    def check_write_post(self, extra={}, is_anonymous=False):
        ...
        self.assertRedirects(
            ...
            target_status_code=302 if is_anonymous else 200,
        )
    ...
    def test_write_post_multirecipient(self):
        ...
        self.assertFormError(
            ...
            "form",
            "recipients",
            CommaSeparatedUserField.default_error_messages["max"].format(
                limit_value=1, show_value=2
            ),
        )
        # authenticated
        self.assertTrue(self.client.login(username="foo", password="pass"))
    ...
    def test_write_notification(self):
        ...
        settings.POSTMAN_AUTO_MODERATE_AS = (
            True  # will generate an acceptance notification
        )
        url = reverse("postman:write")
        data = {"subject": "s", "body": "b", "recipients": self.user2.get_username()}
        self.assertTrue(self.client.login(username="foo", password="pass"))
    ...
    def test_reply_authentication(self):
        ...
        settings.POSTMAN_QUICKREPLY_QUOTE_BODY = (
            True  # no influence here, acts only for Quick Reply
        )
        self.reload_modules()
        response = self.client.get(url)
        self.assertContains(response, 'value="Re: s"')
    ...
    def test_reply_id(self):
        ...
        self.check_reply_404(
            Message.objects.get(pk=self.c12().pk).pk
        ...
        self.check_reply_404(
            Message.objects.get(
                pk=self.create(sender=self.user2, recipient=self.user1).pk
            ).pk
        )
        # cannot reply to a deleted message
        self.check_reply_404(
            Message.objects.get(pk=self.c21(recipient_deleted_at=now()).pk).pk
        )
    ...
    def test_reply_post_filters(self):
        ...
        response = self.client.post(
            reverse("postman:reply_with_user_filter_reason", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_user_filter_no_reason", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_user_filter_false", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_user_filter_exception", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_exch_filter_reason", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_exch_filter_no_reason", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        self.assertFormError(
            response, "form", None, "Writing to some users is not possible: bar."
        )

        response = self.client.post(
            reverse("postman:reply_with_exch_filter_false", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        response = self.client.post(
            reverse("postman:reply_with_exch_filter_exception", args=[pk]),
            data,
            HTTP_REFERER=url,
        )
        self.assertFormError(
        ...
        self.assertFormError(
            response, "form", None, ["first good reason", "anyway, I don't like bar"]
        )
    ...
    def test_view_id(self):
        ...
        self.check_view_404(
            Message.objects.get(pk=self.c23().pk).pk
        )  # create & verify really there
        # existent message but not yet visible to you
        self.check_view_404(
            Message.objects.get(
    ...
    def check_update(self, view_name, success_msg, field_bit, pk, field_value=None):
        ...
        # query string has highest precedence
        response = self.client.post(
            url_with_success_url + "?next=" + redirect_url,
            data,
            HTTP_REFERER="does not matter",
        )
        self.assertRedirects(response, redirect_url)
        # missing payload
    ...
    def check_update_conversation(
        self, view_name, root_msg, field_bit, field_value=None
    ):
...
class MessageManagerTest(BaseTest):
    ...
    def test_query_compiler(self):
        ...
        self.assertListEqual(
            list(qs.filter(recipient_id=2)), [m]
        )  # param 2, must stay at the end
    ...
    def test(self):
        ...
        self.assertQuerysetEqual(
            Message.objects.sent(self.user2, option=OPTION_MESSAGES),
            [m10.pk, m9.pk, m8.pk, m4.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user1, option=OPTION_MESSAGES),
            [m8.pk, m4.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user2, option=OPTION_MESSAGES),
            [m7.pk, m6.pk, m5.pk, m3.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.archives(self.user1, option=OPTION_MESSAGES),
            [],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.archives(self.user2, option=OPTION_MESSAGES),
            [],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.trash(self.user1, option=OPTION_MESSAGES), [], transform=pk
        )
        self.assertQuerysetEqual(
            Message.objects.trash(self.user2, option=OPTION_MESSAGES), [], transform=pk
        )
        # by conversations
        self.assertQuerysetEqual(
        ...
        self.assertQuerysetEqual(
            Message.objects.sent(self.user2),
            [(m10.pk, 0), (m9.pk, 0), (m8.pk, 0), (m4.pk, 1)],
            transform=pk_cnt,
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user1),
            [(m8.pk, 0), (m4.pk, 1)],
            transform=pk_cnt,
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user2),
            [(m7.pk, 0), (m6.pk, 0), (m5.pk, 2)],
            transform=pk_cnt,
        )

        self.assertQuerysetEqual(
            Message.objects.thread(self.user1, Q(thread=m3.pk)),
            [m3.pk, m4.pk, m5.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.thread(self.user1, Q(pk=m4.pk)), [m4.pk], transform=pk
        )
        self.assertQuerysetEqual(
            Message.objects.thread(self.user2, Q(thread=m3.pk)),
            [m3.pk, m4.pk, m5.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.thread(self.user2, Q(pk=m4.pk)), [m4.pk], transform=pk
        )
        ...
        self.assertQuerysetEqual(
            Message.objects.trash(self.user1, option=OPTION_MESSAGES),
            [m8.pk, m3.pk, m2.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
            Message.objects.trash(self.user2, option=OPTION_MESSAGES),
            [m9.pk, m7.pk, m4.pk],
            transform=pk,
        )
        self.assertQuerysetEqual(
        ...
        # by conversations
        self.assertQuerysetEqual(
            Message.objects.sent(self.user1), [(m7.pk, 0), (m5.pk, 1)], transform=pk_cnt
        )
        self.assertQuerysetEqual(
            Message.objects.sent(self.user2), [(m8.pk, 0)], transform=pk_cnt
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user1), [(m4.pk, 1)], transform=pk_cnt
        )
        self.assertQuerysetEqual(
            Message.objects.inbox(self.user2), [(m5.pk, 2)], transform=pk_cnt
        )
        ...
        self.check_status(
            m, status=STATUS_ACCEPTED, is_new=False, recipient_deleted_at=True
        )
        self.check_now(m.read_at)
...
class MessageTest(BaseTest):
    ...
    def check_parties(self, m, s=None, r=None, email=""):
        ...
        self.assertEqual(
            m.admin_recipient(), r.get_username() if r else "<" + email + ">"
        )
    ...
    def test_moderation_from_accepted(self):
        ...
        self.check_status(
            m, status=STATUS_REJECTED, moderation_date=True, recipient_deleted_at=True
        )
        self.check_now(m.moderation_date)
        self.check_now(m.recipient_deleted_at)
    ...
    def test_update_parent(self):
        ...
        r.update_parent(STATUS_REJECTED)
        self.check_status(
            r.parent, status=STATUS_ACCEPTED, thread=parent, is_replied=True
        )
    ...
    def check_notification(
        self, m, mail_number, email=None, is_auto_moderated=True, notice_label=None
    ):
        "Check number of mails, recipient, and notice creation."
        m.notify_users(
            STATUS_PENDING,
            Site.objects.get_current() if Site._meta.installed else None,
            is_auto_moderated,
        )
        ...
        if notification and notice_label:
            if hasattr(
                notification, "Notice"
            ):  # exists for django-notification 0.2.0, but no more in 1.0
    ...
    def test_notification_rejection_user(self):
        ...
        self.check_notification(
            ...
            notice_label="postman_rejection",
        )
    ...
    def test_notification_rejection_user_inactive(self):
        ...
        self.user1.is_active = False
        self.check_notification(
            m, 0, is_auto_moderated=False, notice_label="postman_rejection"
        )
        ...
        seq = (
            ...
            (
                [
                    ...
                    moderate_as_2,
                    moderate_as_50,
                    moderate_as_0_with_reason,
                ],
                "moderate_as_0 with_reason",
            ),
            ...
            (
                [
                    moderate_as_50,
                    moderate_as_none,
                    moderate_badly_as_negative,
                    moderate_badly_as_too_high,
                ],
                True,
            ),
        )
...
class PendingMessageManagerTest(BaseTest):
    ...
    def test(self):
        ...
        msg4 = self.create()
        self.assertQuerysetEqual(
            PendingMessage.objects.all(), [msg4.pk, msg1.pk], transform=lambda x: x.pk
        )
...
class FiltersTest(BaseTest):
    ...
    def test_or_me(self):
        ...
        self.check_or_me("message.obfuscated_sender", "&lt;me&gt;", self.user1, m=m)
        self.check_or_me(
            "message.obfuscated_recipient", "bar@domain.com", self.user1, m=m
        )
    ...
    def test_compact_date(self):
        ...
        self.check_compact_date(
            dt2, dt2.strftime("%d %b").lower()
        )  # filter's 'b' is lowercase
...
class TagsTest(BaseTest):
    ...
    def test_postman_unread(self):
        ...
        self.assertRaises(
            TemplateSyntaxError, self.check_postman_unread, "", self.user1, "As var"
        )
    ...
    def check_order_by(self, keyword, value_list, context=None):
        ...
        self.assertEqual(
            set(r[1:].split("&")), set([k + "=" + v for k, v in value_list])
        )
    ...
    def test_order_by(self):
        ...
        self.check_order_by(
            "subject",
            [("foo", "bar"), (ORDER_BY_KEY, "s"), ("baz", "qux")],
            "foo=bar&" + ORDER_BY_KEY + "=S&baz=qux",
        )
        self.assertRaises(TemplateSyntaxError, self.check_order_by, "", None)
        self.assertRaises(
            TemplateSyntaxError, self.check_order_by, "subject extra", None
        )
...
class UtilsTest(BaseTest):
    ...
    def test_format_body(self):
        ...
        footer = "\n"
        self.assertEqual(
            format_body(self.user1, "foo bar"), header + "> foo bar" + footer
        )
        self.assertEqual(
            format_body(self.user1, "foo bar", indent="|_"),
            header + "|_foo bar" + footer,
        )
        self.assertEqual(
            format_body(self.user1, width=10, body="34 67 90"),
            header + "> 34 67 90" + footer,
        )
        self.assertEqual(
            format_body(self.user1, width=10, body="34 67 901"),
            header + "> 34 67\n> 901" + footer,
        )
        self.assertEqual(
            format_body(self.user1, width=10, body="> 34 67 901"),
            header + "> > 34 67 901" + footer,
        )
        self.assertEqual(
    ...
    def test_get_order_by(self):
        ...
        self.assertEqual(
            ...
            "sender__{0}".format(get_user_model().USERNAME_FIELD),
        )
    ...
    def test_get_user_representation(self):
        ...
        self.assertEqual(
            get_user_representation(self.user1),
            "(u'foo',)" if not six.PY3 else "('foo',)",
        )
        # a function
    ...
    def test_get_user_name(self):
        ...
        settings.POSTMAN_NAME_USER_AS = (
            "id"  # a string is always returned even for not character types
        )
...
class ApiTest(BaseTest):
    ...
    def test_pm_broadcast_multi(self):
        ...
        pm_broadcast(
            sender=self.user1,
            recipients=[self.user2, self.user3],
            subject="s",
            body="b",
        )
        msgs = list(Message.objects.all())
    ...
    def test_pm_write_skip_notification(self):
        "Test the notification skipping."
        pm_write(
            sender=self.user1, recipient=self.user2, subject="s", skip_notification=True
        )
        self.assertEqual(len(mail.outbox), 0)
    ...
    def test_pm_write_auto_archive(self):
        ...
        pm_write(
            sender=self.user1, recipient=self.user2, subject="s", auto_archive=True
        )
        m = Message.objects.get()
        self.check_status(
            m, status=STATUS_ACCEPTED, moderation_date=True, sender_archived=True
        )
    ...
    def test_pm_write_auto_delete(self):
        ...
        m = Message.objects.get()
        self.check_status(
            m, status=STATUS_ACCEPTED, moderation_date=True, sender_deleted_at=True
        )
        self.check_now(m.sender_deleted_at)
    ...
    def test_pm_write_auto_moderators_rejected(self):
        ...
        pm_write(
            ...
            recipient=self.user2,
            subject="s",
            auto_moderators=(lambda m: False,),
        )
        m = Message.objects.get()
        self.check_status(
            m, status=STATUS_REJECTED, moderation_date=True, recipient_deleted_at=True
