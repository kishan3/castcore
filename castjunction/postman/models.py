...
def setup():
    ...
    name_user_as = getattr(
        settings, "POSTMAN_NAME_USER_AS", get_user_model().USERNAME_FIELD
    )
...
class MessageManager(models.Manager):
    ...
    def _folder(self, related, filters, option=None, order_by=None):
        ...
        qs = (
            ...
            else QuerySet(self.model, PostmanQuery(self.model), using=self._db)
        )
        if related:
...
class Message(models.Model):
    ...
    def _obfuscated_email(self):
        ...
        digest = hashlib.md5(
            data.encode()
        ).hexdigest()  # encode(): py3 needs a buffer of bytes
...
class PendingMessageManager(models.Manager):
    ...
    def get_queryset(
        self,
    ):  # changed in Django 1.6: "The get_queryset method was previously named get_query_set."
