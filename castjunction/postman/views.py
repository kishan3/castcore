...
class NamespaceMixin(object):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return super(NamespaceMixin, self).render_to_response(
        ...
        context.update(
            {
                ...
                "by_message_url": reverse(
                    viewname, args=[OPTION_MESSAGES], current_app=current_instance
                ),
...
class WriteView(ComposeMixin, FormView):
    ...
    def get_form_kwargs(self):
        ...
        if (
            isinstance(self.autocomplete_channels, tuple)
            and len(self.autocomplete_channels) == 2
        ):
...
class ReplyView(ComposeMixin, FormView):
    ...
    def get_initial(self):
        self.initial = self.parent.quote(
            *self.formatters
...
class DisplayMixin(NamespaceMixin, object):
    ...
    formatters = (
        format_subject,
        format_body
        if getattr(settings, "POSTMAN_QUICKREPLY_QUOTE_BODY", False)
        else None,
    )
        ...
        context.update(
            {
                ...
                "form": self.form_class(initial=received.quote(*self.formatters))
                if received
                else None,
                "next_url": self.request.GET.get("next")
                or reverse(
                    "postman:inbox", current_app=self.request.resolver_match.namespace
                ),
            }
        )
        return context
