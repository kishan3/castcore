...
class MessageAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if sender_deleted_at and sender_deleted_at < sent_at:
            errors.append(
                ugettext("Deletion date by sender must be later than sending date.")
            )
        ...
        if recipient_deleted_at and recipient_deleted_at < sent_at:
            errors.append(
                ugettext("Deletion date by recipient must be later than sending date.")
            )
        ...
        if replied_at:
            ...
            if replied_at < sent_at:
                errors.append(
                    ugettext("Response date must be later than sending date.")
                )
            if not read_at:
                errors.append(
                ...
                errors.append(
                    ugettext("Response date must be later than reading date.")
                )
...
class MessageAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        obj.notify_users(
            form.initial_status, get_current_site(request), is_auto_moderated=False
        )
