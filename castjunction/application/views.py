...
class ApplicationViewSet(mixins.CreateModelMixin,
    ...
    def create(self, request, job_id=None, **kwargs):
        ...
        if not action:
            ...
            raise ValidationError("Valid actions are '%s' '%s' or '%s'."
                                  % (State.APPLIED, State.PIPELINED, State.IGNORED))
        if created:
            status_i = status.HTTP_201_CREATED
        else:
            status_i = status.HTTP_200_OK
        serializer = self.serializer_class(application, context={'request': self.request})
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.data.get('state'):
            ...
            try:
                ...
                raise ValidationError("Can't switch from state {0} TO {1} "
            ...
            raise ValidationError("Please provide a state to update application status.")
...
class MobileAppVersionView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        version_object = MobileAppVersion.objects.filter(
            version_code__gte=version_code,
            app_type=app_type).last()
...
class MultipleApplicationsViewSet(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for application_id in application_ids:
            applications.extend(Application.objects.filter(
                job__id=data[application_id]['job_id'],
                id=application_id))
        ...
        for application in applications:
            ...
            try:
                ...
                result[application.id] = {
                    "error": "Can't switch from state {0} TO {1} ".format(application.state, state),
                    "user_id": application.user.id}
...
class MultipleUsersViewSet(generics.CreateAPIView):
    ...
    def _validate_target_state(self, target_state):
        ...
        if target_state not in self.acceptable_target_states:
            raise ValidationError("States can be {} only.".format(
                self.acceptable_target_states))
        ...
        for user in users:
            ...
            if application.state == State.SHORTLISTED and target_state == State.INVITED:
                ...
                try:
                    ...
                    result[user.id] = {
                        "error": "Application state can not be {}".format(
                            self.action),
                        "user_id": user.id}
                    continue
            try:
                ...
                if not has_transition_perm(bound_method, request.user):
                    result['errors'].append(
                        {"detail": "You do not have permission to change state from {} TO {}".format(application.state, target_state),
                         "user_id": user.id
                         })
                    continue
                change_of_state(application)
                ...
                result['errors'].append(
                    {"detail": "Can't switch from state {0} TO {1} ".format(application.state, target_state),
                     "user_id": user.id
                     })
