...
class Group(CommonFieldsMixin):
    ...
    start_date = models.DateField(
        null=True, blank=True, help_text="Start date of the project."
    )
...
class Job(CommonFieldsMixin):
    ...
    ages = IntegerRangeField(
        null=True, blank=True, help_text="Age range required for this role."
    )
    required_gender = models.CharField(
        max_length=2, choices=choices.GENDER_CHOICES, default=choices.NOT_SPECIFIED
    )
    required_tokens = models.IntegerField(
        default=5, help_text="How much tokens user needs to have to apply to this job."
    )
    location = models.ForeignKey(
        City, null=True, blank=True, help_text="Location of user's project experience."
    )
    ...
    budgets = IntegerRangeField(
        null=True, blank=True, help_text="budget range amount to be spent on the job."
    )
    ...
    heights = FloatRangeField(
        null=True, blank=True, help_text="Range of height required."
    )
    ...
    audition_range = DateRangeField(null=True, blank=True, help_text="Audition range")
