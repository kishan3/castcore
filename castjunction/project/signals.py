...
def change_job_state(sender, instance, **kwargs):
    ...
    last_status = FieldHistory.objects.filter(field_name='status',
                                              object_id=instance.id).last()
    if last_status and last_status.field_value == choices.APPROVED and instance.status == choices.APPROVED:
...
def incentive_for_job_approved(sender, created, instance, **kwargs):
    ...
    if instance.status == choices.APPROVED:
        ...
        if instance.ages:
            ...
            birth_date_upper_limit = datetime.today() - relativedelta(years=int(lower_age))
            birth_date_lower_limit = datetime.today() - relativedelta(years=int(upper_age))
            users = users.filter(date_of_birth__gte=birth_date_lower_limit.date()).filter(
                date_of_birth__lte=birth_date_upper_limit.date())

        extra_data = ({"extra": {"data": json.dumps({"job_id": instance.id})}})

        bulk_send_push_notification(users, JOB_APPROVED_MESSAGE.format(**{"job_title": instance.title}), **extra_data)
